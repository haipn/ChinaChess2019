...
public class ChessView extends View {
	private Picture[] svgs = null;
	private Picture blackChess = null;
	private Picture redChess = null;
	...
	private static Picture loadBmp(int id, int width, int height) {
//		SvgDrawable svg = SvgDrawable.getDrawable(
//				ChessApplication.getAppContext().getResources(), id);
//		svg.setBounds(0, 0, width, height);
		Picture bmp = Sharp.loadResource(ChessApplication.getAppContext().getResources(), id).getSharpPicture().getPicture();
		return bmp;
	}
	...
	private Picture getBitmap(byte chessman) {
	...
	private void drawChessmen(
		...
		Rect rect = new Rect(cx - (int)(cellSize * 0.97) / 2,
				cy - (int)(cellSize * 0.97) / 2,
...
//					cy - (int)(cellSize * 0.97) / 2, null);
			canvas.drawPicture(redChess, rect);
		} else {
			canvas.drawPicture(blackChess, rect);
//			canvas.drawBitmap(blackChess,
//					cx - (int)(cellSize * 0.97) / 2,
//					cy - (int)(cellSize * 0.97) / 2, null);
		}
		
		Picture txtBmp = getBitmap(chessman);
		canvas.drawPicture(txtBmp, new Rect(
				cx - (int)(cellSize * 0.8) / 2, 
				cy - (int)(cellSize * 0.8) / 2,
				cx + (int)(cellSize * 0.8) / 2,
				cy + (int)(cellSize * 0.8) / 2));
	}
