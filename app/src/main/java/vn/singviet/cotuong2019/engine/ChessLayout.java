...
import vn.singviet.cotuong2019.MainActivity;
...
//	private TextView txtMessage = null;
	...
	public void startServer() {
		final ProgressDialog dialog = ProgressDialog.show(
    			context, null, "Connecting...", true, true, new OnCancelListener() {
...
//    	txtMessage = ((TextView)findViewById(R.id.text_msg));
    ...
    public void newGame() {
    	if (mNewGameListener != null)
    		mNewGameListener.onNewGame();
		AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
		builder.setMessage(R.string.msg_reset_game);
...
//				txtMessage.setText(message);
	...
	public void onSyncPlayerInfo(String player, String partner, int goSide) {
		final String txt = String.format("", player, partner, 
				goSide == 0 ? "A" : "B");
		post(new Runnable() {
			public void run() {
//				txtInfo.setText(txt);
	...
	public void onAskUndo() {
		post(new Runnable() {
			public void run() {
				AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
				builder.setMessage(R.string.msg_undo);
				builder.setPositiveButton(android.R.string.ok, 
						new DialogInterface.OnClickListener() {			
					public void onClick(DialogInterface dialog, int which) {
		...
		post(new Runnable() {
			public void run() {
				if (result) {
					...
					toast.cancel();
					toast.setText(R.string.new_game);
					toast.show();
				}
			}
		});
