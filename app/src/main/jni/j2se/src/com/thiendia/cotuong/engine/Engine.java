...
public class Engine {
	 ...
	 * 
	 * @param direction
	 *            0: red at bottom, 1: black at bottom
	 * @param mv
	 *            MoveInfo object that contained from and to information
	 * @return true: succeeded, false: failed, maybe movement is illegal or can
	 *         not do it in current situation
	 */
	...
	private static native GameState jniGetState(long enginePtr,
			boolean updateBoard);

	/**
	 * Get current game state, and if updateBoard is true will also update board
	 * information.
	 * 
	 * @param updateBoard
	 *            true: update board info, false: no update required
	...
	private static native MoveInfo jniFindSolution(long enginePtr,
			float searchSeconds);
	 ...
	 *            0: red at bottom, 1: black at bottom
	 * @param searchSeconds
	 *            How many seconds would be used.
	 * @return a MoveInfo object that contain the move step, return null if
	 *         failed
	 */
	 ...
	 * @param enumerator
	 */
	synchronized public void enumChessman(IChessmanEnumerator enumerator,
			Object param) {
		if (board == null)
			return;
		for (int y = 0; y < 10; y++) {
	 ...
	 * 
	 * @param direction
	 *            Chess board direction, 0: red on bottom, 1: black on bottom
	 */
